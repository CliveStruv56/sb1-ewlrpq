...
import { MilkOption, MILK_PRICES } from '../types';
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-8">
        {items.map((item) => (
          <div key={`${item.product.id}-${item.options.milk}`} className="flex justify-between mb-3 pb-3 border-b">
            <div>
              ...
              {item.options.milk && (
                <p className="text-sm text-gray-600">
                  ...
                  {MILK_PRICES[item.options.milk as MilkOption] > 0 && 
                    ` (+£${MILK_PRICES[item.options.milk as MilkOption].toFixed(2)})`}
                </p>
              )}
              <div className="flex items-center mt-2">
                <button
                  onClick={() => updateQuantity(item.product.id, item.options.milk, item.quantity - 1)}
